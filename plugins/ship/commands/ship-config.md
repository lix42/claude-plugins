...
The ship workflow caches its Claude Code environment as schema version 6 in
...
1. **Read the existing config** at `.claude/ship.local.json`, if present. Keep its
   values in hand for comparison only when it is valid schema version 6 for the
